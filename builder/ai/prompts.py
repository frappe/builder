...
4. INTERACTIVITY: set_page_script for the behaviours the plan calls for (onload animations, toggles, forms) — one script_type='CSS' script for the styles (reveal/hover classes, @keyframes) and one 'JavaScript' script for the behaviour; never a <style> tag built from JS. Give target elements a class hook first (update_block on the generated blocks) and select that — never a ref.
