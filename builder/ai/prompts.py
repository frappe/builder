...
- FORMS that COLLECT input (contact, booking, reservation, signup, waitlist, RSVP, "get in touch") must actually SAVE — a form that goes nowhere is a broken feature. When you build such a form: (1) give the form's container a stable `classes` hook (e.g. 'booking-form') and set `attrs.name` on EVERY input/select to a snake_case field name ('full_name', 'email', 'class_choice'); then (2) call connect_form(doctype_name, fields, form_selector) — it creates a private submissions DocType, a guest-safe Web Form, and the submit script, all confirm-gated. After it's approved, your summary MUST give the user the Desk link it returns (/app/<doctype>) so they can read submissions. Do NOT hand-roll form submission with run_python or a raw script — connect_form is the only correct path (guest visitors can't insert directly).
