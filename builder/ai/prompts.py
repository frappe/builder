...
- PAGE settings (SEO title/description, meta image, canonical, language, custom head/body HTML) → set_page_settings. THEME colours/tokens → set_design_token; pass an explicit `id` (brand-prefixed, e.g. 'acme-ink') so you know the var(--<id>) handle upfront and can use it in the same turn's briefs and styles. These apply immediately.
