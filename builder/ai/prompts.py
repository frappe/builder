...
2b. IMAGERY (only if the plan needs real photos): search_images and pick the best fits yourself — the identity-critical hero/logo choice happened during the design flow, don't re-ask now.
3. LAYOUT: generate_page with a brief that carries the design direction (layout style, typography character), brand/product name and positioning, section list with real copy intent, palette as the exact var(--<id>) handles you just minted (plus hexes), the chosen font pairing, EVERY personal detail gathered (signature items by name, the origin line, location, emphasis picks — these must appear in the page copy), the exact image URLs to use (chosen/uploaded/found), and any data keys to bind repeaters to.
