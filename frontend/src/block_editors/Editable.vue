<template>
	<div class="editor group relative cursor-pointer" :class="{'border-2 border-blue-400': store.selected_component && store.selected_component.element_id === elementProperties.id}">
		<component v-if="elementProperties.node_type !== 'Text'" :is="elementProperties.element"
			...
			@click.exact.stop="select_component($event, elementProperties)"
			@dblclick.stop
			draggable="true" v-bind="{...elementProperties.attributes, ...elementProperties.skipped_attributes}" :style="elementProperties.styles" ref="component">
