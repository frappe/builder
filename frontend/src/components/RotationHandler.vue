...
import { getElementRectInEditor, getEventPointInEditor } from "@/utils/canvasFrameDom";
...
const handleRotate = (ev: MouseEvent, baseAngle: number) => {
	// the drag runs on the editor document, so the block is measured there too
	const bounds = getElementRectInEditor(props.target as Element);
	const centerX = bounds.left + bounds.width / 2;
	const centerY = bounds.top + bounds.height / 2;
	const startPoint = getEventPointInEditor(ev);
	let previousPointerAngle = Math.atan2(startPoint.y - centerY, startPoint.x - centerX) * (180 / Math.PI);
