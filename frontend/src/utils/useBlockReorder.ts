...
export function startBlockReorder(event: MouseEvent, block: Block, breakpoint?: string) {
	...
	// blocks render inside a canvas frame, so they are looked up in its root
	const frameRoot = canvasStore.activeCanvas?.canvasProps?.frameRoots?.get(dragBreakpoint);

	const getContainerEl = (target: Block): HTMLElement | null =>
		(frameRoot ?? document).querySelector(
	...
	// The pointer runs in editor coordinates and the blocks are measured in their
	// own frame, so points cross over at the two boundaries: hit testing and
	// measurement go into the frame, the ghost and the drop indicator come back out.
	const frameDocument = sourceEl.ownerDocument;
	const toFramePoint = (point: CanvasFramePoint): CanvasFramePoint => {
		const frame = getFrameElement(frameDocument);
		return frame ? editorPointToFrame(frame, point) : point;
	};

	// the press can come from inside a canvas frame (an editor overlay), so it is
	// read in editor coordinates like every later pointer position
	const startPoint = getEventPointInEditor(event);
	const startX = startPoint.x;
	const startY = startPoint.y;
	const sourceRect = getElementRectInEditor(sourceEl);
	const sourceSize = sourceEl.getBoundingClientRect();
	...
	const resolveTargetContainer = (clientX: number, clientY: number) => {
		const el = frameDocument.elementFromPoint(clientX, clientY) as HTMLElement | null;
	...
	const updateTarget = (clientX: number, clientY: number) => {
		...
		const containerRect = getElementRectInEditor(parentEl);
		const line = computeDropIndicator(lines, index, cr, style, direction, {
			width: sourceSize.width,
			height: sourceSize.height,
		});
		const linePoint = framePointToEditor(frameDocument, { x: line.left, y: line.top });
		...
		// the indicator is drawn over the editor, the geometry was measured in the frame
		t.line = { ...line, left: linePoint.x, top: linePoint.y, length: line.length * getScale() };
	};
	...
	const onMove = (e: MouseEvent) => {
		...
		if (!started) {
			if (Math.abs(point.x - startX) < DRAG_THRESHOLD && Math.abs(point.y - startY) < DRAG_THRESHOLD) {
				return;
			}
			beginDrag();
		}
		...
		const framePoint = toFramePoint(point);
