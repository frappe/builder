...
export function useSpacingHandler(getTargetBlock: () => Block, getBreakpoint: () => string) {
	...
	const startSpacingDrag = (
		...
		// the handle lives in the canvas frame, so drag points come back in canvas pixels
		startCanvasDrag(event, handle, {
			cursor: getComputedStyleFor(handle).cursor,
			onMove: ({ event: moveEvent, movementX, movementY }) => {
				onUpdate?.();
				const delta = toLocalDelta(movementX, movementY, getRotation());
